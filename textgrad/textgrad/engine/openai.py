        ...
        if not base_url:
            if not resolved_api_key:
                raise ValueError(
                    ...
                    "if you'd like to use OpenAI models."
                )
            self.client = OpenAI(api_key=resolved_api_key)
        elif base_url and base_url == OLLAMA_BASE_URL:
            # Ollama uses a dummy key
            self.client = OpenAI(base_url=base_url, api_key="ollama")
        else:
            # Support OpenAI-compatible providers (e.g., OpenRouter) via base_url + api_key
            if not resolved_api_key:
                raise ValueError(
                    "Missing API key for custom base_url. Set OPENAI_API_KEY or pass api_key=..."
                )
