...
# NOTE:
# LiteLLM (experimental engine) pulls in a large dependency chain and can be slow to import.
# Do NOT import it at module import time. We'll import it only if the user explicitly requests
# an "experimental:" engine.
LiteLLMEngine = None  # type: ignore
...
def get_engine(engine_name: str, **kwargs) -> EngineLM:
    ...
    if engine_name.startswith("experimental:"):
        ...
        try:
            from textgrad.engine_experimental.litellm import LiteLLMEngine as _LiteLLMEngine  # type: ignore
        except Exception as e:
            raise ImportError(
                "LiteLLMEngine is not available. Install/enable LiteLLM dependencies or avoid using "
                "'experimental:' engines."
            ) from e
        return _LiteLLMEngine(model_string=engine_name, **kwargs)
