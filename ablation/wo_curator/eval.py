...
def load_full_transcript_text(path: Path) -> str:
    if not path.exists():
        return ""
    return path.read_text(encoding="utf-8").strip()

def normalize_items_from_suggestions(payload: Dict[str, Any]) -> List[Dict[str, Any]]:
    """
    Keep evaluation criteria unchanged, only flatten wo_curator output format:
    results[].suggestions[] -> one item per suggestion.
    """
    ...
    for res in results:
        ...
        if isinstance(nested, list) and nested:
            for sug in nested:
                ...
                normalized.append(
                    {
                        "knowledge_point_id": res.get("knowledge_point_id"),
                        "topic": res.get("topic", ""),
                        "student_current_ci": res.get("student_current_ci", ""),
                        "target_mode": res.get("target_mode", ""),
                        "suggestion_priority": sug.get("priority"),
                        "gap_diagnosis": sug.get("gap_diagnosis", ""),
                        "proposed_rewrite": sug.get("proposed_rewrite", ""),
                        "scaffolding_strategy": sug.get("scaffolding_strategy", ""),
                        "new_question": sug.get("new_question", ""),
                        "reference_answer": sug.get("reference_answer", ""),
                        "_raw_parent": res,
                        "_raw_suggestion": sug,
                    }
                )
...
{{
  ...
  "consistency_checks": {{
    "student_current_ci": "{str(item.get('student_current_ci', '')).lower()}",
    "target_mode": "{str(item.get('target_mode', '')).lower()}",
    "is_student_ci_consistent_with_rederived": true,
...
def _is_actionable_enough(scaffolding_strategy: str) -> bool:
    ...
    if not text:
        return False
    has_actor = any(k in text for k in ["teacher", "student", "students", "教师", "学生"])
    has_action = any(
        k in text
        for k in [
            "ask", "prompt", "write", "discuss", "assign", "provide", "collect", "summarize",
            "debate", "reflect", "引导", "提问", "书写", "讨论", "组织", "分配", "反馈", "总结"
        ]
    )
    has_timing = any(k in text for k in ["after", "before", "during", "each", "weekly", "课前", "课中", "课后", "每次", "之后"])
    return has_actor and has_action and has_timing

def apply_verifiable_caps(result: Dict[str, Any], item: Dict[str, Any], segment_text: str) -> Dict[str, Any]:
    """
    Minimal hard-check layer:
    - No verifiable quote evidence in gap_diagnosis => accuracy capped at 3.
    - Scaffolding lacks actor+action+timing => actionability capped at 3.
    """
    ...
    if not _has_verifiable_quote_in_transcript(gap_text, segment_text):
        try:
            scores["accuracy"] = min(float(scores.get("accuracy", 0) or 0), 3.0)
        except (TypeError, ValueError):
            scores["accuracy"] = 3.0
        flags.append("gap_diagnosis_no_verifiable_quote")
        issues.append("gap_diagnosis lacks verifiable quoted evidence that can be matched in transcript.")

    # 2) Executability cap for actionability
    if not _is_actionable_enough(scaffold_text):
        try:
            scores["actionability"] = min(float(scores.get("actionability", 0) or 0), 3.0)
        except (TypeError, ValueError):
            scores["actionability"] = 3.0
        flags.append("scaffolding_missing_actor_action_timing")
        issues.append("scaffolding_strategy misses one or more of actor/action/timing elements.")

    # Recompute overall and verdict with same standard (unchanged policy)
    acc = float(scores.get("accuracy", 0) or 0)
    align = float(scores.get("alignment_grounding", 0) or 0)
    act = float(scores.get("actionability", 0) or 0)
...
def evaluate_one(
    ...
    class_kp_map: Dict[int, Dict[str, Any]], full_transcript_text: str,
    ...
    seg_text = (
        segment_text_map.get(kp_id, "")
        or str(class_kp_map.get(kp_id, {}).get("text", "")).strip()
        or full_transcript_text
    )
    ...
    try:
        ...
        res = apply_verifiable_caps(res, item, seg_text)
        res["efficiency"] = {
            **judge["efficiency"],
...
def main() -> None:
    ...
    items = normalize_items_from_suggestions(sug_payload)
    if not isinstance(items, list): raise ValueError("Invalid evaluation data: 'results' must be a list.")

    # Important: lock input sources to the fixed paths configured above.
    # Do NOT read dynamic paths from icap_suggestions.json metadata.
    segment_text_map: Dict[int, str] = {}
    full_transcript_text = load_full_transcript_text(INPUT_TRANSCRIPT_FULL_TXT)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        future_to_idx = {
            executor.submit(
                ...
                class_kp_map, full_transcript_text, a_feature_map, expected_kp_map, expected_global_config
