...
Step 1 (Level):
- Infer current cognitive mode from transcript evidence, then choose one target_mode following ICAP hierarchy (I > C > A > P).
- Output this in gap_diagnosis explicitly.
Step 2 (Breadth):
- Evaluate participation breadth and explain possible classroom coverage bottlenecks using transcript evidence only.
...
- In that case, proposed_rewrite and scaffolding_strategy MUST focus on:
  "how to scale this already-achieved cognitive mode to broader participants."
...
def main() -> None:
    ...
    print("[1/3] Loading knowledge segmentation data...")
